...
/**
 * Comprehensive Test Script for Phase 2 Advisor Functions
 * Tests all 5 advisor-specific functions with real data
 */
...
function timeFunction<T>(fn: () => T): { result: T; time: number } {
...
function getTestAdvisorId(): number | null {
  try {
...
// ============================================
function testGetAdvisorStudentList(advisorId: number): TestResult {
  console.log('\n📋 Test 1: getAdvisorStudentList');
  console.log('─'.repeat(80));

  try {
    const { result: students, time } = timeFunction(() =>
      db.prepare(`
        SELECT
          s.id as student_id,
...
// ============================================
function testGetHighestGPAStudent(advisorId: number): TestResult {
  console.log('\n🏆 Test 2: getHighestGPAStudent');
  console.log('─'.repeat(80));

  try {
    const { result: student, time } = timeFunction(() =>
      db.prepare(`
        SELECT
          s.student_id as student_number,
          u.full_name,
    ...
    if (passed) {
      ...
      const actualHighest = Math.max(...allGPAs.map((s: any) => parseFloat(s.gpa)));
...
// ============================================
function testGetHonorStudents(advisorId: number, minGPA: number = 3.5): TestResult {
...
// ============================================
function testGetStudentsByGPA(advisorId: number, threshold: number, comparison: 'below' | 'above'): TestResult {
  ...
  try {
    ...
    if (students.length > 0) {
      ...
      const allCorrect = students.every((s: any) => {
...
// ============================================
function testGetLastStudentContact(advisorId: number, studentName: string): TestResult {
  console.log(`\n💬 Test 5: getLastStudentContact (searching for: "${studentName}")`);
  console.log('─'.repeat(80));

  try {
    // Step 1: Find student
    const { result: student, time: findTime } = timeFunction(() =>
      db.prepare(`
        SELECT
          s.id as student_id,
          s.student_id as student_number,
    ...
    const { result: lastConversation, time: convTime } = timeFunction(() =>
      db.prepare(`
        SELECT
    ...
    const { result: lastAIChat, time: aiTime } = timeFunction(() =>
      db.prepare(`
        SELECT
