...
function generateStudentID(level, section, index) {
...
function getInstructorByDepartment(department) {
...
function hasScheduleConflict(existingSchedules, newSchedule) {
...
function seedDatabase() {
    try {
        ...
        for (const table of tables) {
            try {
                database_1.db.exec(`DROP TABLE IF EXISTS ${table}`);
            }
            catch (error) {
        ...
        const schemaPath = path_1.default.join(__dirname, '../../database/schema-sqlite.sql');
        ...
        const levels = database_1.db.prepare('SELECT * FROM levels ORDER BY level_number').all();
        const sections = database_1.db.prepare('SELECT * FROM sections ORDER BY id').all();
        console.log(`\nFound ${levels.length} levels and ${sections.length} sections`);
        // ====================
        // 1. CREATE ADMIN USER
        // ====================
        console.log('\n--- Creating Admin User ---');
        const adminResult = database_1.db.prepare('INSERT INTO users (email, password_hash, full_name, user_type) VALUES (?, ?, ?, ?)').run('admin@mentorlink.com', passwordHash, 'System Administrator', 'admin');
        ...
        for (const level of levels) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                // Create user
                const userResult = database_1.db.prepare('INSERT INTO users (email, password_hash, full_name, user_type) VALUES (?, ?, ?, ?)').run(email, passwordHash, fullName, 'advisor');
                const userId = userResult.lastInsertRowid;
                // Create advisor record
                const advisorResult = database_1.db.prepare('INSERT INTO advisors (user_id, level_id, specialization, is_available) VALUES (?, ?, ?, ?)').run(userId, level.id, specialization, 1);
                advisorIds.push(advisorResult.lastInsertRowid);
        ...
        const insertUserStmt = database_1.db.prepare('INSERT INTO users (email, password_hash, full_name, user_type) VALUES (?, ?, ?, ?)');
        const insertStudentStmt = database_1.db.prepare('INSERT INTO students (user_id, student_id, birthdate, level_id, section_id, gpa, attendance_percentage) VALUES (?, ?, ?, ?, ?, ?, ?)');
        const insertCourseStmt = database_1.db.prepare(`
    ...
    `);
        const insertAssignmentStmt = database_1.db.prepare('INSERT INTO advisor_assignments (student_id, advisor_id) VALUES (?, ?)');
        const countAssignmentsStmt = database_1.db.prepare('SELECT COUNT(*) as count FROM advisor_assignments WHERE advisor_id = ?');
        for (const level of levels) {
            console.log(`\nLevel ${level.level_number}:`);
            ...
            for (const section of levelSections) {
                ...
                for (let i = 1; i <= 20; i++) {
                    ...
                    while (assignedCourses.length < numCourses && attempts < maxAttempts) {
                        ...
                        const schedule = { time: timeSlot.time, days };
                        ...
                        insertCourseStmt.run(studentDbId, course.name, course.code, instructor.name, instructor.email, timeSlot.time, days, location.room, location.building, course.creditHours, grade, semester, course.department, course.description, course.prerequisites || null);
                        assignedCourses.push(course);
                        studentSchedule.push(schedule);
                        totalCourses++;
                    }
                    // Auto-assign to advisor (load-balanced: least assigned)
                    const levelAdvisorIds = advisorIds.slice((level.level_number - 1) * 6, level.level_number * 6);
                    const advisorLoads = levelAdvisorIds.map((advisorId) => {
                        const result = countAssignmentsStmt.get(advisorId);
                        return { advisorId, count: result.count };
                    });
                    const leastLoadedAdvisor = advisorLoads.reduce((min, current) => current.count < min.count ? current : min);
        ...
        const allStudents = database_1.db.prepare('SELECT * FROM students ORDER BY RANDOM() LIMIT 50').all();
        let conversationCount = 0;
        const getAssignmentStmt = database_1.db.prepare('SELECT advisor_id FROM advisor_assignments WHERE student_id = ?');
        const getAdvisorStmt = database_1.db.prepare('SELECT user_id FROM advisors WHERE id = ?');
        const insertConversationStmt = database_1.db.prepare('INSERT INTO conversations (student_id, advisor_id, status) VALUES (?, ?, ?)');
        const insertMessageStmt = database_1.db.prepare('INSERT INTO messages (conversation_id, sender_id, message_text, is_read) VALUES (?, ?, ?, ?)');
        for (const student of allStudents) {
            const assignment = getAssignmentStmt.get(student.id);
            if (assignment) {
                const advisorId = assignment.advisor_id;
                const advisor = getAdvisorStmt.get(advisorId);
                const convResult = insertConversationStmt.run(student.id, advisorId, randomElement(['active', 'active', 'active', 'resolved']));
                ...
                for (let i = 0; i < numMessages; i++) {
                    ...
                    insertMessageStmt.run(conversationId, senderId, randomElement(sampleMessages), Math.random() > 0.3 ? 1 : 0);
                }
                conversationCount++;
            }
        }
        ...
        const randomStudents = database_1.db.prepare('SELECT * FROM students ORDER BY RANDOM() LIMIT 100').all();
        let aiChatCount = 0;
        const insertAIChatStmt = database_1.db.prepare('INSERT INTO ai_chat_history (student_id, user_message, ai_response) VALUES (?, ?, ?)');
        for (const student of randomStudents) {
            ...
            for (let i = 0; i < numChats; i++) {
                insertAIChatStmt.run(student.id, randomElement(sampleUserMessages), randomElement(sampleAIResponses));
                aiChatCount++;
            }
        }
