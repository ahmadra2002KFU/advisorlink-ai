...
try {
  const schema = db.prepare('PRAGMA table_info(staff_contacts)').all() as any[];
...
try {
  const countResult = db.prepare('SELECT COUNT(*) as count FROM staff_contacts').get() as any;
...
try {
  ...
  samples.forEach((staff: any, index: number) => {
    console.log(`\n   ${index + 1}. ${staff.name}`);
