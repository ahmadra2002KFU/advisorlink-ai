...
/**
 * Test script for staff contacts search functionality
 * Tests various query scenarios to verify the function works correctly
 */
...
function searchStaffContacts(query: string): any {
  console.log(`\n🔍 Searching for: "${query}"`);
  console.log('-------------------------------------------');
...
// Summary statistics
const totalStaff = db.prepare('SELECT COUNT(*) as count FROM staff_contacts').get() as any;
const categories = db.prepare('SELECT DISTINCT category FROM staff_contacts ORDER BY category').all() as any[];

console.log('📊 Database Summary:');
console.log(`   Total Staff: ${totalStaff.count}`);
console.log(`   Categories: ${categories.map((c: any) => c.category).join(', ')}`);
console.log('');
