...
async function searchFacilities(searchTerm: string): Promise<any> {
...
async function simulateAIConversation(studentQuery: string, searchTerm: string) {
  console.log('\n' + '='.repeat(80));
  ...
  if (result.success && result.facility) {
    ...
    if (f.capacity) console.log(`👥 ${f.capacity}`);
    console.log(`📧 Contact: ${f.contact}`);
    console.log(`📞 Phone: ${f.phone}`);

    if (f.services && f.services.length > 0) {
      console.log(`\n✨ Services offered:`);
      f.services.forEach((service: string) => console.log(`   • ${service}`));
    }
    ...
    result.facilities.forEach((f: any, i: number) => {
...
async function runDemo() {
  ...
  await simulateAIConversation(
    "Where is the main computer lab? I need to work on my programming assignment.",
    "main computer lab"
  );
  ...
  await simulateAIConversation(
    "I need to get my transcript. Where do I go?",
    "registrar"
  );
