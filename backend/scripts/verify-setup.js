...
function verifySetup() {
    ...
    try {
        ...
        const dbPath = process.env.DB_PATH || path_1.default.join(__dirname, '../../mentorlink.db');
        ...
        const tables = database_1.db.prepare(`
    ...
    `).all();
        ...
        if (missingTables.length > 0) {
            ...
            const advisorCount = database_1.db.prepare('SELECT COUNT(*) as count FROM advisors').get();
            const studentCount = database_1.db.prepare('SELECT COUNT(*) as count FROM students').get();
            const faqCount = database_1.db.prepare('SELECT COUNT(*) as count FROM faqs').get();
